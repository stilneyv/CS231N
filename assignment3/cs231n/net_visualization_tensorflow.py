...
def compute_saliency_maps(X, y, model):
    ...
    with tf.GradientTape() as tape:
        ...
        # b
        y_hat = model(X)
        correct_scores = tf.gather_nd(y_hat, tf.stack((tf.range(N), y), axis=1))
        loss = correct_scores
        # 3. find gradient
        grads = tape.gradient(loss, X)
        # 4. compute map, needs to be ndarray for autograder ugh
        saliency = np.amax(np.abs(grads), axis=3)
...
def make_fooling_image(X, target_y, model):
    ...
    # a
    X = tf.convert_to_tensor(X)
    target_y = tf.convert_to_tensor(target_y, dtype=tf.int64)
    ...
    for i in range(max_iters):
        with tf.GradientTape() as tape:
            ...
            if tf.math.argmax(y_hat,axis=1) == target_y:
                break
            loss = y_hat[:,target_y]
            # print(loss)
            # 3. find gradient
...
def class_visualization_update_step(X, model, target_y, l2_reg, learning_rate):
    ...
    with tf.GradientTape() as tape:
        tape.watch(X)
        y_hat = model(X)
        loss = y_hat[0,target_y] - 2 * l2_reg * tf.nn.l2_loss(X)
        # 3. find gradient
        grads = tape.gradient(loss, X)
        X = X + learning_rate * grads[0]
