...
def discriminator(seed=None):
    ...
    model = tf.keras.models.Sequential([
        tf.keras.layers.Dense(256,input_shape=(784,), activation=tf.nn.leaky_relu),
...
def discriminator_loss(logits_real, logits_fake):
    ...
    DG = tf.nn.sigmoid_cross_entropy_with_logits(logits = logits_fake, labels = tf.zeros_like(logits_fake))
    D = tf.nn.sigmoid_cross_entropy_with_logits(logits = logits_real, labels = tf.ones_like(logits_real))

    loss = tf.reduce_mean(DG) + tf.reduce_mean(D)
...
def dc_discriminator():
    ...
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Conv2D(filters=32, kernel_size=(5,5), strides=(1, 1), padding='valid', activation=tf.nn.leaky_relu),
        # tf.keras.layers.LeakyReLU(alpha=0.01),
        tf.keras.layers.MaxPool2D(pool_size=(2, 2), strides=2),
        tf.keras.layers.Conv2D(64, kernel_size=(5,5), strides=(1, 1), padding='valid', activation=tf.nn.leaky_relu),
        # tf.keras.layers.LeakyReLU(alpha=0.01),
        tf.keras.layers.MaxPool2D(pool_size=(2, 2), strides=2),
...
def dc_generator(noise_dim=NOISE_DIM):
    ...
    model = tf.keras.models.Sequential([
        ...
        # tf.keras.layers.ReLU(),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Conv2DTranspose(filters=1, kernel_size=(4,4), strides=(2, 2), padding='same', activation=tf.nn.tanh)
        # tf.keras.layers.Activation('tanh')
    ])
