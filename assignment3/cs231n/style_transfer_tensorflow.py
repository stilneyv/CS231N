...
def tv_loss(img, tv_weight):
    ...
    # keep in mind you lose 1 dim from H and W
    dif1 = (img[:, :-1, :, :] - img[:, 1:, :, :])**2
    dif2 = (img[:, :, :-1, :] - img[:, :, 1:, :])**2
    loss = tv_weight*(tf.reduce_sum(dif1) + tf.reduce_sum(dif2))
    return loss
...
def gram_matrix(features, normalize=True):
    ...
    x = tf.reshape(features, (-1, C))
    gram = tf.linalg.matmul(x, x, transpose_a=True)
    if normalize:
      gram /= (H*W*C)
